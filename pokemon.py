...
def get_pokemon_details():
    ...
    for name, id in favorite_pokemon_dict.items():
        ...
        pokemon_details_2 = requests.get(f'https://pokeapi.co/api/v2/pokemon-species/{id}/').json()

        # Keep base happiness of all 5 fav pokemon to calculate avg and median later
        base_happiness.append(pokemon_details_2['base_happiness'])
        ...
        pokemons.append({
            ...
            'color': pokemon_details_2['color']['name'],
            'moves': random.sample([move['move']['name'] for move in pokemon_details['moves']], 2),
            'base_happiness': pokemon_details_2['base_happiness']
        })
